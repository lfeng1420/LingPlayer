[简体中文](README.zh_cn.md) | [繁體中文](README.zh_tw.md) | [日本語](README.ja_jp.md) | [한국어](README.ko_kr.md) | [Deutsch](README.de_de.md) | [Français](README.fr_fr.md) | [Español](README.es_es.md) | [Português](README.pt_br.md) | [Polski](README.pl_pl.md) | [Русский](README.ru_ru.md) | [Türkçe](README.tr_tr.md)

# Ling Player

## Description

Ling Player is a cross-platform local music player supporting dozens of common audio formats including FLAC, APE, DSD, MP3, and AAC. It features CUE sheet parsing, grouped shuffle playback, and more. With GPU-accelerated real-time visualizer and a transparent glass UI paired with custom background images, Ling Player delivers a rich, personalized music experience.

## ✨ Core Features

- Support for dozens of audio formats including FLAC/APE/DSD/MP3/AAC
- Intelligent CUE sheet parsing
- Real-time visualizer effects
- Transparent glass effect + custom background images
- Grouped shuffle playback
- Steam Rich Presence

## 📥 Get

- [Steam](https://store.steampowered.com/app/4496780/)
- [Microsoft Store](https://www.microsoft.com/store/apps/9PGF663WTBPZ?ocid=badge)

## 🌍 Localization

Ling Player supports multiple language interfaces. Contributions are welcome!

- [Translation Guide](sources/Languages/README.md)
- [Contributors](sources/Languages/CONTRIBUTORS.md)

## 🔗 Links

- 🛡️ [Privacy Policy](https://www.lfengs.com/en-us/apps/ling/privacy-policy.html)
- 🚀 [Changelog](https://www.lfengs.com/en-us/apps/ling/changelog.html)
