[简体中文](README.zh_cn.md) | [English](README.md) | [日本語](README.ja_jp.md) | [한국어](README.ko_kr.md) | [Deutsch](README.de_de.md) | [Français](README.fr_fr.md) | [Español](README.es_es.md) | [Português](README.pt_br.md) | [Polski](README.pl_pl.md) | [Русский](README.ru_ru.md) | [Türkçe](README.tr_tr.md)

# 輕靈音樂

## 簡介

輕靈音樂是一款跨平台本地音樂播放器，支援 FLAC、APE、DSD、MP3、AAC 等數十種常見音訊格式，具備 CUE 分軌解析、分組隨機播放等實用功能。配合即時渲染的可視化頻譜與透明毛玻璃介面，搭配自訂背景圖片，帶來豐富的個人化體驗。

## ✨ 核心功能

- 支援 FLAC/APE/DSD/MP3/AAC 等數十種音訊格式
- CUE 分軌檔案智慧解析
- 即時渲染的可視化頻譜效果
- 透明毛玻璃效果 + 自訂背景圖片
- 分組隨機播放
- Steam 豐富狀態顯示

## 📥 取得

- [Steam](https://store.steampowered.com/app/4496780/)
- [Microsoft Store](https://www.microsoft.com/store/apps/9PGF663WTBPZ?ocid=badge)

## 🌍 在地化

輕靈音樂已支援多語言介面，歡迎參與翻譯貢獻！

- [翻譯指南](sources/Languages/README.md)
- [貢獻者列表](sources/Languages/CONTRIBUTORS.md)

## 🔗 相關連結

- 🛡️ [隱私權協議](https://www.lfengs.com/zh-tw/apps/ling/privacy-policy.html)
- 🚀 [更新日誌](https://www.lfengs.com/zh-tw/apps/ling/changelog.html)
