[简体中文](README.zh_cn.md) | [English](README.md) | [繁體中文](README.zh_tw.md) | [日本語](README.ja_jp.md) | [Deutsch](README.de_de.md) | [Français](README.fr_fr.md) | [Español](README.es_es.md) | [Português](README.pt_br.md) | [Polski](README.pl_pl.md) | [Русский](README.ru_ru.md) | [Türkçe](README.tr_tr.md)

# Ling Player

## 소개

Ling Player는 FLAC, APE, DSD, MP3, AAC 등 수십 가지의 일반적인 오디오 형식을 지원하는 크로스 플랫폼 로컬 음악 플레이어입니다. CUE 시트 파싱, 그룹 셔플 재생 등의 실용적인 기능을 갖추고 있습니다. GPU 가속 실시간 비주얼라이저와 투명 글래스 UI에 커스텀 배경 이미지를 더해 풍부한 개인화 경험을 제공합니다.

## ✨ 주요 기능

- FLAC/APE/DSD/MP3/AAC 등 수십 가지 오디오 형식 지원
- 지능형 CUE 시트 파싱
- 실시간 비주얼라이저 효과
- 투명 글래스 효과 + 커스텀 배경 이미지
- 그룹 셔플 재생
- Steam 리치 프레즌스 표시

## 📥 다운로드

- [Steam](https://store.steampowered.com/app/4496780/)
- [Microsoft Store](https://www.microsoft.com/store/apps/9PGF663WTBPZ?ocid=badge)

## 🌍 현지화

Ling Player는 다국어 인터페이스를 지원합니다. 번역 기여를 환영합니다!

- 📖 [번역 가이드](sources/Languages/README.md)
- 👥 [기여자 목록](sources/Languages/CONTRIBUTORS.md)

## 🔗 관련 링크

- 🛡️ [개인정보 처리방침](https://www.lfengs.com/ko-kr/apps/ling/privacy-policy.html)
- 🚀 [변경 로그](https://www.lfengs.com/ko-kr/apps/ling/changelog.html)
