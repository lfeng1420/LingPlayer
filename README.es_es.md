[简体中文](README.zh_cn.md) | [English](README.md) | [繁體中文](README.zh_tw.md) | [日本語](README.ja_jp.md) | [한국어](README.ko_kr.md) | [Deutsch](README.de_de.md) | [Français](README.fr_fr.md) | [Português](README.pt_br.md) | [Polski](README.pl_pl.md) | [Русский](README.ru_ru.md) | [Türkçe](README.tr_tr.md)

# Ling Player
---

## Descripción

Ling Player es un reproductor de música local multiplataforma compatible con docenas de formatos de audio comunes como FLAC, APE, DSD, MP3 y AAC. Ofrece análisis de archivos CUE, reproducción aleatoria agrupada y más. Con un visualizador en tiempo real acelerado por GPU y una interfaz de vidrio transparente con imágenes de fondo personalizables, Ling Player brinda una experiencia musical rica y personalizada.

## ✨ Funciones principales

- Compatibilidad con docenas de formatos de audio como FLAC/APE/DSD/MP3/AAC
- Análisis inteligente de archivos CUE
- Efectos de visualizador en tiempo real
- Efecto de vidrio transparente + imágenes de fondo personalizables
- Reproducción aleatoria agrupada
- Steam Rich Presence

## 📥 Obtener

- [Steam](https://store.steampowered.com/app/4496780/)
- [Microsoft Store](https://www.microsoft.com/store/apps/9PGF663WTBPZ?ocid=badge)

## 🌍 Localización

Ling Player es compatible con múltiples idiomas de interfaz. ¡Las contribuciones son bienvenidas!

- 📖 [Guía de traducción](sources/Languages/README.md)
- 👥 [Colaboradores](sources/Languages/CONTRIBUTORS.md)

## 🔗 Enlaces

- 🛡️ [Política de privacidad](https://www.lfengs.com/es-es/apps/ling/privacy-policy.html)
- 🚀 [Registro de cambios](https://www.lfengs.com/es-es/apps/ling/changelog.html)
